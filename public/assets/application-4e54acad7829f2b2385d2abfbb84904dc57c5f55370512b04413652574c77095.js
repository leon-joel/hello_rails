...
(function() {
  ...
  $(document).on('ready page:load', function() {
    var DIM_DELTA_X, DIM_DELTA_Y, DIM_FIRST, MAX_LEVEL, dim_x, dim_y, gameStart, games, init_cells, level, penarty, t1;
    $(document).on('click', '#getResult', function() {
      var chara, charactors, name, rand, rand2, result, tweetLink, type, types;
      name = $("#name").val();
      if (name === "") {
        alert("名前を入力してください!");
        return;
      ...
      $("#result").text(result);
      tweetLink = "<a href='https://twitter.com/intent/tweet?text=" + encodeURIComponent(result) + "&hashtags=dotinstall' target='_blank'>" + "ツイートする</a>";
      return $("#tweet").html(tweetLink);
    });
    level = 0;
    games = [['微', '徴'], ['大', '太'], ['未', '末'], ['白', '日'], ['吉', '古'], ['0', 'O']];
    ...
    gameStart = function() {
      var cells, dummy, dummy_offset, i, is_hint, j, k, l, m, n, offset, ref, ref1, ref2, results, seikai, seikai_char;
      dummy = games[level][0];
      ...
      if (is_hint) {
        for (i = j = 0, ref = Math.pow(level + 1, 2); 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
          dummy_offset.push(Math.floor(Math.random() * (dim_x * dim_y)));
        }
      }
      cells = '';
      seikai_char = is_hint ? "seikai_char" : "";
      for (k = m = 0, ref1 = dim_x * dim_y - 1; 0 <= ref1 ? m <= ref1 : m >= ref1; k = 0 <= ref1 ? ++m : --m) {
        if (k === offset) {
          cells += "<span class=\"chars " + seikai_char + "\" id=\"s" + k + "\">" + seikai + "</span>";
        } else if (indexOf.call(dummy_offset, k) >= 0) {
      ...
      $('#cells').html(cells);
      results = [];
      for (l = n = 0, ref2 = dim_x * dim_y - 1; 0 <= ref2 ? n <= ref2 : n >= ref2; l = 0 <= ref2 ? ++n : --n) {
        results.push($('#s' + l).click(function() {
          var elapse, t2;
          if ($(this).text() === seikai) {
            if (level < MAX_LEVEL) {
              level++;
            ...
            penarty += 1;
            return $('#penarty').text("（ペナルティ " + penarty + " 秒）");
          }
        }));
      }
      return results;
    };
    return init_cells = function() {
      $("#target_string").text("");
      $('#penarty').text("");
      $('#cells').html("");
      level = 0;
...
(function() {
  $(document).on('ready page:load', function() {
    var compare, sortOrder;
    compare = function(a, b, type, col) {
      var _a, _b;
    ...
    sortOrder = 1;
    return $('.dothesort').click(function() {
      var $rows, arrow, col, type;
      type = $(this).data('type');
      col = $(this).index();
      $rows = $('tbody>tr');
